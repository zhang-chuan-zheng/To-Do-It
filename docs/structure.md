...
| 文件 | 用途与允许编辑内容 |
| --- | --- |
| `.gitattributes` | 固定仓库换行与二进制识别规则：C++、QML、CMake、配置、文档和文本型 Qt 资源使用 LF，CSV 与 Windows 脚本使用 CRLF，图片、字体、压缩包及编译产物禁止文本转换；新增文件类型时按实际格式扩展。 |
...
| `assets/icons/default/arrow-right.svg` | 底部“打赏一下”纯文字后指向打赏图标按钮的青色箭头 SVG；仅作无背景视觉提示，不持有点击区域。 |
...
| `assets/icons/default/donate.svg` | 底部状态栏右侧打开打赏图片弹层的心形加号 SVG；只表示入口，不包含收款信息。 |
| `assets/icons/default/folder.svg` | 文件夹附件缩略图及“添加文件夹”选项使用的青色文件夹 SVG。 |
| `assets/icons/default/maximize.svg` | 窗口最大化操作的矢量图标。 |
| `assets/icons/default/minimize.svg` | 窗口最小化操作的矢量图标。 |
| `assets/icons/default/palette.svg` | 备注工具栏的调色盘 SVG；点击后打开应用内 RGB/CMYK 调色盘，只作用于当前选中文字。 |
...
| `assets/images/README.md` | 非图标图片的默认/自定义根目录和单文件回退边界。 |
...
| `config/default-resources.json` | 定义 Logo、图标、图片、背景的默认/自定义根，并登记当前石墨界面使用的 Logo、文件夹、打赏入口、右向箭头和二维码占位图等稳定资源键；已删除停用的液态主题专用键，相对路径以安装目录解析。 |
...
| `src/presentation/qml/pages/MainPage.qml` | 装配标题栏、信息栏、表头、事件列表和带打赏入口的帮助行；把 `QuoteData` 的随机正文/作者和换句接口接到信息栏，并用仅在窗口可见且非最小化时运行的单个 2 分钟计时器触发自动换句；注入 CSV 事件与 UUID 工厂，转发快照给 `EventData.saveEvents()`，保存失败时保留脏标记并显示错误，区分文件/文件夹打开提示，同时协调页面级点击外部失焦。 |
...
| `src/presentation/qml/components/EventCard.qml` | 单个事项的完整圆角边界及整卡拖放/悬停/右键命中区；右键只打开 `EventContextMenu`，不再直接发出删除。只读 `pointerHovered` 将 `cardHover.hovered` 的实时值提供给事件树，统一 `editingLocked` 汇总名称、开始时间、完成时间、状态和备注（含调色盘）编辑状态，并通过 `editingStateChanged()` 通知全局排序协调器。空草稿的 `draftExitTimer` 把名称框焦点作为强保护条件，名称框得到焦点时立即停止计时，只有名称为空、鼠标离开、名称框失焦、输入法未组合且其他字段均未编辑时才允许 800 ms 后取消。详情整体展开/收起保留高度动画，但 `noteEditor.editing` 为真时禁用外卡高度动画，使连续换行或删除时正文与蓝色外框同步变化。`cardDrag.parent` 指向完整 `EventCard`，因此备注折叠时覆盖顶部事项行，展开后同时覆盖附件、备注工具栏、正文和详情空白。拖拽热点直接使用相对于整卡的 `centroid.pressPosition`，防止从详情区起拖时产生投放偏移。`childDropActive` 只在 `dropMode=child` 时让整卡显示强调背景和焦点边框；`ordinaryHoverActive` 只在没有投放模式时保留普通悬停效果，before/after 模式因此不会误亮整卡，只在相应边缘显示 3 px 插入线。拖动时把当前委托提升到列表顶层并在原位置保留低对比占位，`dragProxy` 显示含序号、名称和落位提示的浮动卡。 |
| `src/presentation/qml/components/EventContextMenu.qml` | 事项右键弹层；复用 `PopupGlassBackground`、主题颜色、圆角和字体，提供“新建子事项”“删除事项”两个选项，并将位置限制在事项可用范围内；只发出请求信号，不直接修改模型。 |
| `src/presentation/qml/components/EventDeleteConfirmation.qml` | 模态删除确认弹层；显示目标标题和子孙数量，明确整棵子树一并删除且不可撤销，提供取消与危险色删除按钮；确认信号由事件树执行模型事务。 |
| `src/presentation/qml/components/EventTableHeader.qml` | 绘制无子框的加高七列表头，以轻微纵向线标示列边界，紧凑排序箭头紧贴名称，并与列表共用滚动条槽和列宽；向事件树发送排序方向。 |
| `src/presentation/qml/components/EventTreeView.qml` | 用 `initialEvents` 替换空源模型，不含内置事项；负责筛选、搜索、动画排序、UUID 草稿、时间、文件/文件夹附件和唯一备注。视口级空白点击命中检查排除事项卡与添加按钮，卡片间、按钮下方和空列表区域均可发出全局失焦请求。备注输入只写源模型，退出编辑后再同步可见模型，避免每个字符反向覆盖正在输入的 `TextArea`。开始/完成时间或状态提交后立即重算持续时间；单个 60 秒计时器只批量更新源模型和可见模型的显示字段，不重排、不标脏。持续时间排序生效时，开始/完成时间修改只先同步当前可见字段并建立待排序请求；`durationSortDelayTimer` 只有在所有事项的名称、时间、状态、备注等编辑器以及 Windows 原生附件选择器都连续空闲 500 ms 后，才按完整排序流水线执行一次动画重排。任一编辑器重新激活或任一字段再次提交修改都会停止并重置倒计时。详情状态机通过 `isEventCardHovered()` 在展开、切换与收起计时器到期时重新读取当前委托的真实悬停状态；当 `editingDetailsEventId` 指向另一事项时，`requestDetailsHover()` 只记录高亮和候选事项并停止切换计时，`activateDetails()` 也以同一门禁拒绝释放焦点或展开。编辑结束后若候选事项仍真实悬停，重新等待完整 240 ms 才切换；`scheduleDetailsCloseIfInactive()` 统一检查悬停、编辑和附件固定锁，`releasePinnedDetails()` 在附件选择取消或展示期结束后重新安排收起，避免缓存状态导致偶发常驻。`requestAttachmentSelection()` 在同步调用 `AttachmentDialog.chooseAttachments()` 前固定事项并设置全局编辑门禁，返回后接收系统选择结果、显示平台错误或按取消路径解锁；目录 URL 规范为以分隔符结尾，打开时区分类型。跟踪拖动目标模式，完成成为子项/同级移动/移回一级并拒绝后代投放，落下后保持同一委托并对 `x`、`width`、`y` 和轻微缩放执行约 260 ms 落位过渡；修改只标脏，5 分钟到期时仅在脏状态发出完整持久快照，关闭时再次检查并可强制刷新。 |
...
| `src/presentation/qml/components/InfoFilterBar.qml` | 名人名言占据最小宽度 300 px 的左侧弹性区域，右侧放置彼此独立的筛选框和统计框；公开名言/作者、动态筛选选项、待完成/已完成计数 alias，不自行遍历事件。 |
| `src/presentation/qml/components/QuoteBanner.qml` | 将控制器提供的名言和作者保持在同一行、水平左对齐并仅做垂直居中；`quoteViewport` 直接填满 `GlassPanel` 已应用内边距的内容区，禁止再次按根尺寸叠加顶部间距。正常宽度不放大、缩窄时自适应减小；属性变化时让旧内容移动接近框底、缩小到 56% 并淡出，再让新内容从接近框顶的 56% 尺寸向中心落下放大，裁剪在圆角区域并使用 `Motion.quoteRollDuration`；双击只发出换句请求，不直接解析 JSON 或管理周期。 |
...
| `src/presentation/qml/components/StatusSummary.qml` | 根据外部真实计数固定显示“`N 项待完成 · M 项已完成`”，不再显示百分比；可编辑格式，不遍历事件或补祖先。 |
...
| `src/presentation/qml/effects/GlassSurface.qml` | 绘制当前石墨主题统一透明 tint、圆角和完整细描边；只执行轻量颜色动画，不捕获或模糊桌面。 |
| `src/presentation/qml/effects/PopupGlassBackground.qml` | 截取弹层覆盖的 `MainPage` 区域并用单个 `MultiEffect` 有限模糊，再叠加 `Theme.popupTint`、当前表面色和描边；用于高层弹窗，不替代 Windows 桌面磨砂。 |
| `src/presentation/qml/effects/README.md` | 说明 `GlassSurface`、应用内弹层模糊与 Windows 桌面 Acrylic/DWM 之间的职责边界，并禁止在每个事项字段中执行实时高斯模糊。 |
| `src/presentation/qml/pages/README.md` | 页面级布局、信号转发和不持有业务状态的装配边界。 |
| `src/presentation/qml/styles/Metrics.qml` | 单例保存窗口尺寸、6 px 缩放热区、38 px 顶部栏、40 px 信息栏、24 px 窗口按钮/搜索锚点、22 px Logo、64 px 标题搜索间距、460 px 搜索展开宽度、34 px 添加入口、32 px 附件缩略图、圆角、列宽、加高表头、搜索条件宽度，以及 14 px 固定滚动槽、9 px 滑块宽度和 12% 最小滑块比例；禁止业务状态。 |
| `src/presentation/qml/styles/Motion.qml` | 单例保存通用、220 ms 备注高度、搜索、260 ms 重排/落位、150 ms 拖动预览收窄、170 ms 单阶段名言滚动动画、缓动曲线、800 ms 搜索与空草稿离开延迟及减少动画开关；可编辑动效令牌。 |
| `src/presentation/qml/styles/IconCatalog.qml` | 单例集中公开应用标志、搜索、窗口、下拉、日期、排序、添加事项、附件/文件夹、调色盘、打赏入口、箭头和打赏图片 URL；不再登记液态主题专用资源，以后资源定位器可覆盖这些属性。 |
| `src/presentation/qml/styles/Theme.qml` | 单例保存石墨/预留 aurora 颜色令牌和 `useVariant()` 扩展接口；当前界面固定使用石墨变体且没有运行时切换控件，可编辑主题数据但不得写事件逻辑。 |
...
| `updater/CMakeLists.txt` | 构建 `ToDoIt::UpdateCore` 和控制台 `ToDoItMigrator`，公开传递 Qt Core 与 Infrastructure；因为公开的迁移接口使用 `CsvParseResult`，其消费者必须继承对应头文件目录。迁移器安装到主程序旁。 |
...
作用：统一不同 Git 客户端、编辑器和 Windows 开发环境中的仓库换行规则，避免 `core.autocrlf` 造成反复警告或无意义整文件差异。

主要逻辑：C/C++、QML、CMake、QML Project、JSON、XML、模板、Markdown、SVG 与 YAML 等文本统一以 LF 存入并检出；应用 CSV 按仓储编码器的 RFC 4180 约定使用 CRLF；PowerShell、Batch 和 CMD 脚本使用 CRLF；图片、字体、PDF、压缩包、可执行文件、动态库、静态库和调试符号显式标记为二进制。可编辑内容仅限新增真实文件类型或调整已确认的格式约定，不得把未知二进制类型强制声明为文本。
...
作用：存放可替换、可打包但不包含用户隐私的视觉与文本资源。Logo、图标、打赏图片和背景图保持独立资源，以便用户替换；程序可内置最小应急资源，但不得覆盖用户文件。
...
默认主题令牌：颜色、渐变、玻璃 tint、内外描边、边缘高光/阴影、圆角、字号、间距、动画时长和可选背景图片路径。只存数据，不存 QML/JavaScript 逻辑。
...
首次运行复制为 `data/config/resources.json`。顶层字段为 `schemaVersion`、`logos`、`icons`、`images` 和 `backgrounds`；资源分组包含 `defaultRoot` 与允许为空的 `customRoot`，前三组的 `files` 把稳定逻辑键映射到相对资源文件名。`images.donationQr` 当前指向明确标注的占位 SVG，替换正式二维码只修改这一逻辑键，不得在组件中另设路径旁路。
...
- `statusSummary()`：从全部正式事项计算全局“待完成/已完成”计数，不受状态筛选、搜索文本或祖先补全影响；已取消不计入待完成，每个父子事项独立计算一次。
...
- `decodeUtf8()`：先把 `std::string` 复制为具有完整表达式后生命周期的 `QByteArray`，再立即把 `QStringDecoder::decode()` 的惰性结果实体化为 `QString`，最后检查解码错误；禁止把临时 `QByteArray` 直接传给解码器后延迟转换，否则会产生悬空引用并可能误读 `schema_version`。
...
- `loadConfiguration(resourcesJson, installRoot)`：读取 `logos/icons/images/backgrounds` 四组根目录，把相对路径固定解析到安装目录并校验规范化结果。
...
- `recalculate(allEvents)`：计算全部正式事项的待完成数与已完成数；已取消不计入待完成，祖先上下文不重复计数。
- `summaryText()`：固定输出“`N 项待完成 · M 项已完成`”，不再生成百分比或筛选状态名称。
...
当前逻辑：创建 1360×820、最小 1100×680 的圆角无边框 `ApplicationWindow`，默认使用最终设计稿的石墨色系。底色和低饱和灰青、暗红、蓝灰环境光先在不可直接显示且启用纹理层的 `backgroundSource` 中合成；不可见、启用纹理层且抗锯齿的 `roundedWindowMask` 提供与 `Metrics.radiusLarge` 一致的 alpha 轮廓；最外层 `MultiEffect` 只启用 `maskEnabled` 且关闭自动留白，把完整背景严格裁切到圆角内。这里不得改回 `Rectangle.clip`，因为普通 QML 裁剪不跟随 `radius`。随后装配带 `id` 的 `MainPage`；只读 `popupBackdropSource` 将该页面提供给应用内下拉弹层截取，最上层 `WindowResizeHandles` 为四边和四角补齐系统鼠标缩放。本阶段未连接 `WindowEffectsController`，没有 Acrylic/DWM；实色背景仍是平台效果接入前的明确回退，不得把应用内弹层模糊称为真实桌面磨砂。

可编辑内容：顶层窗口约束、应用级快捷键和页面装配。

### `src/presentation/qml/pages/MainPage.qml` `[现有 CSV 展示页面]`

按垂直顺序组合 `FramelessTitleBar`、`InfoFilterBar`、事件列表 `GlassPanel` 和 `BottomHelpBar`；事件列表内部再组合 `EventTableHeader` 与 `EventTreeView`。页面导入 `ToDoIt.Controllers 1.0`，把 `QuoteData.quote/author` 绑定给信息栏并将双击换句转发到 `QuoteData.refreshQuote()`；单一 `quoteRefreshTimer` 每 2 分钟调用同一入口，`running` 只在宿主窗口可见且 `visibility !== Window.Minimized` 时为真，因此最小化期间不换句、恢复后重新计满周期。加载诊断进入默认底栏提示。页面同时将 `EventData.events` 和 `createEventId()` 工厂交给事件树，并把动态筛选选项、全局待完成/已完成计数绑定回信息栏。事件树发出完整快照时调用同步 `EventData.saveEvents()`；失败则重新置脏并优先显示 `EventData.saveError`。`flushPendingChanges()` 供根窗口关闭前刷新；页面仍通过 `pointInsideItem()`、`clearFocusWhenTappedOutside()`、延后 `TapHandler` 和 `releaseInputFocusRequested()` 协调输入焦点，不直接解析或写 CSV/JSON。
...
| `RoundIconButton.qml` | 已实现图标接口 | `iconSource`、`iconSize`、兼容性 `iconText`、`helpText`、`destructive`、`chromeStyle` 和 `helpVisibilityChanged()`；chrome 按钮静止时透明，悬停、按下或键盘焦点时才显示统一石墨表面 |
| `FramelessTitleBar.qml` | 已实现主页面交互 | 无外层面板、底框或额外 level，表现为窗口原生顶层菜单栏；当前不装配主题切换入口。公开 `pointInsideSearch()` 和 `backgroundPressed()` 协调点击外部失焦，移动超过系统阈值后才调用 `startSystemMove()`；继续提供搜索信号与 SVG 窗口按钮 |
...
| `QuoteBanner.qml` | 已实现主页面视觉与换句过渡 | `quote`、`author`、`refreshRequested()`；显示 `QuoteData` 经页面注入的名言和作者并强制单行，正常宽度不放大，窗口缩小时可降至极小字号。属性变化时先把旧内容向下移动、缩放至 0.76 并淡出，再替换文本并从上方放大淡入；动画裁剪在本组件内，连续请求只保留最新待显示内容，双击只发出换句请求，不读取数据文件或自行计时 |
...
| `StatusSummary.qml` | 已实现动态统计展示 | `pendingCount`、`completedCount`；固定格式化为“`N 项待完成 · M 项已完成`”，不再显示百分比或筛选状态名称，不含演示默认值，也不遍历事件 |
| `InfoFilterBar.qml` | 已实现动态绑定接口 | 左侧单行名言的最小宽度从 360 px 收紧为 300 px，右侧彼此分离的筛选框和统计框；公开 `quote`、`quoteAuthor`、`filterOptions`、`pendingCount`、`completedCount` alias 及筛选/名言/帮助信号，不计算事件数据 |
...
| `EventTreeView.qml` | 已实现 CSV 展示、交互与快照保存协调 | `replaceSourceEvents()` 装载真实快照且代码内无示例，并先以当前中国时间刷新持续时间。根级 `TapHandler` 配合 `pointHitsEventContent()` 让事项卡间、添加按钮下方和空列表视口点击释放输入焦点；不会替代事项卡或添加按钮自身交互。`refreshStatusMetadata()` 独立计算全局待完成与已完成计数，已取消不计入待完成，筛选/搜索不改变统计。`beginDraft(parentId)` 同时支持一级草稿和右键子事项草稿，保持全局单草稿限制；`requestDelete()` 收集标题与完整子孙数量，`confirmPendingDelete()` 先构造稳定 ID 集合再逆序删除整棵子树，随后重建模型、标脏并等待 5 分钟/关闭保存。右键菜单与删除确认可见时纳入全局编辑门禁，事项详情收起逻辑同时排除当前上下文菜单来源。备注输入期间只更新源模型，`updateDetailsEditing(..., false)` 在退出编辑后同步可见备注，避免模型回写干扰当前 `TextArea`。任一事项仍在编辑时，其他事项的悬停只记录普通高亮和候选 ID，`activateDetails()` 不释放焦点也不切换备注；编辑结束且候选事项仍悬停时重新等待 240 ms 后才展开，快速划过始终不会展开。`durationForTimes()` 计算完成/未完成/尚未开始文本；开始时间、完成时间或状态提交成功后立即更新，统一 60 秒计时器只同步源模型与可见模型字段，不重建排序、不标脏。持续时间排序生效时，时间字段提交不会立刻重建模型：`durationSortPendingAfterTimeEdit` 记录待排状态，`anyEventContentEditing()` 扫描全部可见事项的统一 `editingLocked` 并检查附件/上下文菜单/删除确认状态，`scheduleDurationSortAfterEdit()` 与 `durationSortDelayTimer` 只在所有事项内容连续停止编辑 500 ms 后启动位置动画并重建一次；编辑焦点在事项或字段间转移以及再次提交修改时都会停止并重新等待。`requestAttachmentSelection()` 固定当前事项、启用编辑门禁并同步调用 Windows `AttachmentDialog`；返回后添加系统选择结果、显示平台错误或按取消路径解锁。动态生成状态、筛选树和编号。`isEventCardHovered()` 通过 `eventRepeater.itemAt()` 读取委托实时悬停状态；`scheduleDetailsCloseIfInactive()` 统一检查悬停、编辑、附件固定和上下文菜单锁后安排收起。`attachSelectedEntries()` 去重文件/目录 URL、为目录补结尾分隔符并只在确有新增时标脏；`openAttachment()` 区分目录提示。`persistentSnapshot()` 排除空草稿，5 分钟仅在脏状态发出快照；记录 `dragHoverMode` 和 `settlingEventId`，完成子项/同级/一级移动后让委托的横向缩进、宽度、纵向位置和缩放连续过渡 |
| `EventCard.qml` | 已实现主页面交互 | 完整事项外框是悬停、右键、左键激活和拖放命中区；右键释放输入焦点并打开 `EventContextMenu`，分别发出新建子事项或请求删除信号，不直接修改模型。`contextMenuStateChanged()` 让事件树在菜单打开时锁定当前详情位置。只读 `pointerHovered` 对外暴露当前 `HoverHandler` 状态，统一 `editingLocked` 合并名称、开始/完成时间、状态和备注（含调色盘）的编辑状态，`editingStateChanged()` 把变化交给事件树的全局 500 ms 排序门禁。详情整体展开/收起使用 `Motion.detailsDuration`，备注正在编辑时禁用 `implicitHeight` 动画，使正文与事项边框随换行和删除同步伸缩。`DragHandler.parent=root` 让命中区随事项卡高度覆盖展开后的附件与备注框，5 px 阈值区分普通点击和拖动，热点直接使用整卡坐标。`childDropActive` 与 `ordinaryHoverActive` 把投放高亮和普通悬停分离：child 才整卡高亮且必然成为子事项，before/after 只显示对应 3 px 边缘插入线。拖动期间把委托 `z` 提升并在原位覆盖低对比占位，可见 `dragProxy` 通过 `Drag.source` 保留事件 ID，并显示当前落位模式。 |
| `EventContextMenu.qml` | 已实现事项右键菜单 | `openAt()` 把 164 px 弹层定位在右键位置并限制边界；复用 `PopupGlassBackground`，两个 34 px 选项分别发出 `createChildRequested()` 和 `deleteRequested()`，危险操作使用主题危险色，不触碰事件模型 |
| `EventDeleteConfirmation.qml` | 已实现删除二次确认 | `eventTitle`、`descendantCount`、`deletionConfirmed()`；模态圆角磨砂弹层明确提示完整子树删除范围，取消仅关闭，确认信号先发出再关闭，模型和持久化由事件树统一负责 |
...
| `DonationPopup.qml` | 已实现图片弹层 | `qrSource` 默认取 `IconCatalog.donationQr`；以当前主题材质显示等比二维码/占位图片、标题、关闭入口和替换说明，不处理支付或网络请求 |
...
- `GlassSurface.qml` `[现有]`：`surfaceLevel`、`interactive`、`fillColor`；为当前石墨主题绘制圆角透明 tint 与完整细描边，只对填充和描边做轻量颜色动画，不捕获或模糊桌面。
- `PopupGlassBackground.qml` `[现有]`：使用 `ShaderEffectSource` 截取弹层覆盖的 `MainPage` 局部，再由单个 `MultiEffect` 模糊并覆盖 `Theme.popupTint`、当前表面色与描边。`backdropSource`、`surfaceLevel`、`blurAmount` 是允许调整的视觉接口；它只用于应用内高层浮层，不能冒充 Windows Acrylic/DWM。
...
- `Theme.qml`：QML 单例，默认启用最终设计稿的 `graphite` 石墨主题，同时保留 `aurora` 作为未来扩展示例；`useVariant(id)` 是代码级扩展接口。此前的 `liquidGlass` 变体和 `toggleLiquidGlass()` 已删除，当前界面不提供运行时主题切换。
- `Metrics.qml`：1360×820 默认窗口、1100×680 最小窗口、6 px 窗口缩放热区、46 px 顶部栏、40 px 信息栏、30 px 窗口按钮、32 px 搜索锚点、28 px Logo、64 px 标题到搜索间距、460 px 搜索展开宽度、34 px 添加入口、统一圆角/紧凑间距，以及外卡到内框 6 px、字段间 4 px、事项间 8 px、32 px 附件缩略图、14 px 滚动槽、9 px 滚动条、12% 最小滑块比例和七列表格列宽。
- `Typography.qml`：默认使用 `Microsoft YaHei UI`，集中正文/辅助/标题字号和字重；`noteSize=15` 让备注明显但不过度地大于普通正文，名言由组件在正常字号上只缩不放。
- `Motion.qml`：`reducedMotion`、快/普通/搜索动画时长、220 ms 备注展开时长、260 ms 列表重排与拖放落位时长、150 ms 拖动代理收窄时长、360 ms 单阶段（约 720 ms 完整）名言滚动时长、800 ms 搜索收起延迟和统一缓动曲线。
- `IconCatalog.qml`：可写 URL 属性集中对应稳定应用标志、搜索、窗口控制、下拉、日期、排序、添加事项、附件添加、文件夹、调色盘、打赏入口、右向指示箭头和打赏图片；不再包含液态主题专用资源，后续由资源定位器按自定义根目录逐文件覆盖。
...
构建 `todoit_update_core` 静态库和别名 `ToDoIt::UpdateCore`。Qt Core 与 `ToDoIt::Infrastructure` 都是公开依赖：`EventCsvMigrator.h` 的函数签名直接使用 Infrastructure 的 `CsvParseResult`，因此链接 UpdateCore 的测试或其他消费者必须同步获得其头文件搜索路径。`ToDoItMigrator` 是无 Qt Quick 的控制台目标，安装在主程序旁供 IFW 调用。
...
当前主页面已经显示无外框顶层菜单栏、可替换 SVG Logo、随机单行哲学名言、窗口按钮、四边四角鼠标缩放、只在移动超过系统阈值后启动的标题栏拖动、460 px 搜索、覆盖卡片间与列表末尾空白的点击外部失焦、带页面模糊与石墨遮罩的统一高层弹层、等高信息栏、紧凑排序表头、层级事项卡、独立字段框、重要度、可编辑状态、固定数字槽时间、直接子项比例、220 ms 平滑且目标锁定的单一备注区域、覆盖滚动视口完整高度的 9 px 主题滚动条、Windows Shell 原生附件选择，以及同时显示 20 px 加号图片和“添加事项”文字的 34 px 添加入口。顶部栏由 50 px 压缩到 46 px，但 Logo 恢复为 28 px、搜索锚点恢复为 32 px、窗口按钮恢复为 30 px并统一垂直居中；信息栏压缩到 40 px，名言最小宽度减为 300 px，右侧显示“`N 项待完成 · M 项已完成`”。名言启动时随机选择，双击区域时重新随机且避免紧邻重复；窗口可见且非最小化时每 2 分钟自动切换，文本水平、垂直居中，旧句用 360 ms 向下接近框底并缩小淡出，新句再用 360 ms 从框顶落至中心并放大显现，最小化期间暂停，资源异常时底栏显示诊断并保留回退文本。右键新建子事项后名称框会取得焦点并停止空草稿退出计时，焦点仍在名称框时不会因为鼠标尚未移动到子事项而被删除。窗口底色与环境光使用单一圆角 alpha 蒙版整体裁剪，不再由矩形 `clip` 放任左上和右下的对角环境光进入透明角区。详情展开、切换和收起计时器会重新读取当前事项卡的真实悬停值，附件选择取消、右键菜单关闭或短暂固定期结束也会统一重新判断收起条件，避免过期悬停 ID 造成备注偶发常驻。整张事项卡均可开始拖动，展开后命中范围同步覆盖附件和备注详情；拖动时显示跟随指针的浮动事项卡，只有 child 目标会整卡高亮并成为子事项，before/after 同级目标只显示对应边缘的 3 px 插入线；松开后真实卡片以约 260 ms 同时过渡缩进、宽度和位置。事项任意位置右键打开统一磨砂菜单，可新建以当前事项为父节点的草稿，或经显示完整子孙数量的二次确认后删除整棵子树。附件入口不显示二级菜单或应用自绘浏览器；Windows 原生窗口支持多选文件，并可通过同窗按钮添加当前文件夹，已添加的文件和文件夹均双击打开。缩略图为 32 px，“附件”与缩略图顶部对齐；备注标题为 13 px、正文使用 15 px 独立字号，编辑时正文与外框同步伸缩且不接受可见模型逐字符回写，B 与 RGB/CMYK 调色盘只格式化当前选中文字；底部可复制提示右侧的“打赏一下”文字与箭头均无背景且不可点击，只有末尾圆角正方形图标按钮可打开可替换的打赏图片弹层。当前不加载液态玻璃材质，也不显示主题切换开关。

当前事项已接通 `event.csv` 双向快照：启动时从可执行文件相对 `data/event.csv` 读取，开发启动可用 `TODOIT_EVENT_FILE` 覆盖；目标不存在时先原子创建父目录与表头-only 空文件，已有文件绝不覆盖。CSV 字段解码现保持输入 `QByteArray` 生命周期直到 `QString` 实体化，修复合法 v1 文件的 `schema_version` 偶发误判，并在真正版本不支持时显示期望值和原始值。严格加载失败时列表为空、底部显示文件/行/列诊断并锁止写回。有效字段编辑、备注格式、文件/文件夹附件关联/解绑、右键新建子事项、右键确认删除子树及树拖放只标记待保存；文件夹沿用 `attachments_json` 绝对路径数组并保留目录结尾分隔符，无需数据架构迁移。开始时间、完成时间或状态提交后持续时间立即重算；统一 60 秒计时器只刷新未完成事项的内存显示值，不触发排序、保存或 CSV 架构变化。持续时间排序生效时，开始/完成时间的修改先建立待排序请求；所有事项的名称、时间、状态、备注编辑器、上下文菜单、删除确认与 Windows 原生附件选择过程必须连续空闲 500 ms 才执行一次动画重排，期间任一内容重新编辑或再次提交修改都会停止并重置等待。单个 5 分钟计时器仅在确有修改时生成不含空草稿的完整快照，正常关闭前再次检查并只刷新未落盘修改。拖动整卡可成为目标子事项、插入同级前后或投放到添加入口移回一级，目标为自身后代时拒绝。正式运行期资源定位器、真实打赏二维码、删除前普通保存备份与可恢复事务、完整富文本白名单净化、普通文件系统缩略图和平台磨砂仍未接入。

本次 0.2.1 改动已把 Windows 安装 bindir 调整到主程序根目录，新增运行库/插件静态打包门禁、完整安装器 QSS、版本化欢迎页、卸载数据选择/二次确认/成功后清理、各模式中文状态和新版本清单；本轮继续接通随机名言、可见窗口每 2 分钟滚动换句、紧凑顶部/信息栏、全局待完成/已完成统计、事项右键新建子事项、事项右键删除完整子树与二次确认、持续时间即时/分钟刷新、持续时间排序的全局 500 ms 事项编辑门禁、Windows RC EXE 图标、Qt IFW 安装器图标、Windows 原生附件选择、列表空白区失焦、备注即时高度同步和编辑模型隔离，同时保留既有 GitHub 在线/离线更新与字段迁移事务。全部修改遵循用户的“只编辑文件”要求；只做文件内容、资源键和职责边界的静态检查，没有重新配置、编译、运行、执行 QML lint、测试、CMake Install、调用 Qt IFW 或生成安装包。上述“已完成”只代表源码、脚本、模板与文档已经静态写入，不代表这些改动已经运行验证。
